# Customized auth.py for the Open Pandora handheld
import socket
import os
import subprocess
import webbrowser
...
class GoogleAuth(ApiAttributeMixin, object):
  ...
  def LocalWebserverAuth(self, host_name='localhost',
                         port_numbers=[8080, 8090], browsercmd=""): # added the pnd_run cmd of the browser as an argument here
    ...
    ffcmd=browsercmd+ ' -a "'+authorize_url+'"' # this will run the browser .pnd and pass the authorize_url
    print ffcmd
    proc = subprocess.Popen(ffcmd, shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE)
